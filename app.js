...
function calcular(){
  ...
  const ato = num('ato');
  const financiamento = num('financiamento');
  ...
  inters.filter(x=>x.valor>0).forEach(x=>{
    linhas.push(`• ${x.data || 'Intermediária'}: *${money(x.valor)}*`);
  });
  ...
  if(financiamento > 0) linhas.push(`• Financiamento bancário: *${money(financiamento)}*`);
  ...
  linhas.push('');
  linhas.push('_Valores sujeitos à confirmação, disponibilidade da unidade, correções previstas em contrato e, quando houver financiamento, análise e aprovação de crédito._');
